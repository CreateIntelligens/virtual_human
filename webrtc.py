    ...
    async def next_timestamp(self) -> Tuple[int, fractions.Fraction]:
        ...
        if self.kind == 'video':
            if hasattr(self, "_timestamp"):
                ...
                print(f'[{datetime.now()}] video start:',self._start)
            ...
            if hasattr(self, "_timestamp"):
                ...
                self._timestamp = 0
                self.timelist.append(self._start)
                print(f'[{datetime.now()}] audio start:',self._start)
    ...
    async def recv(self) -> Union[Frame, Packet]:
        ...
        if self.kind == 'video':
            ...
            if self.framecount==100:
                print(f'[{datetime.now()}] ------actual avg final fps:{self.framecount/self.totaltime:.4f}')
                self.framecount = 0
                self.totaltime=0
        return frame
...
class HumanPlayer:
    ...
    def __log_debug(self, msg: str, *args) -> None:
        logger.debug(f'[{datetime.now()}] HumanPlayer {msg}', *args)
