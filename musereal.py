...
class MuseReal(BaseReal):
    ...
    def process_frames(self,quit_event,loop=None,audio_track=None,video_track=None):
        while not quit_event.is_set():
            ...
            # First validate all array indices
            try:
                if idx >= len(self.frame_list_cycle):
                    print(f'[{datetime.now()}] Warning: Invalid frame index {idx} >= {len(self.frame_list_cycle)}')
                    continue
