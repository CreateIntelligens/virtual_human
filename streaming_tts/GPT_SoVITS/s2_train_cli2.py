...
import utils_cli as utils
hps = utils.get_hparams(stage=2)
# Set exp_dir to match s2_ckpt_dir
exp_dir = os.path.dirname(hps.s2_ckpt_dir)
if not os.path.exists(exp_dir):
    os.makedirs(exp_dir, exist_ok=True)
hps.data.exp_dir = exp_dir
...
def run(rank, n_gpus, hps):
    ...
    try:  # 如果能加载自动resume
        _, _, _, epoch_str = utils.load_checkpoint(
            utils.latest_checkpoint_path(hps.s2_ckpt_dir, "D_*.pth"),
            net_d,
            optim_d,
        )  # D多半加载没事
        if rank == 0:
        ...
        _, _, _, epoch_str = utils.load_checkpoint(
            utils.latest_checkpoint_path(hps.s2_ckpt_dir, "G_*.pth"),
            net_g,
            optim_g,
        )
    ...
    if epoch % hps.train.save_every_epoch == 0 and rank == 0:
        if hps.train.if_save_latest == 0:
            utils.save_checkpoint(
                ...
                epoch,
                os.path.join(
                    hps.s2_ckpt_dir, "G_{}.pth".format(global_step)
                ),
            )
            utils.save_checkpoint(
                net_d,
                ...
                epoch,
                os.path.join(
                    hps.s2_ckpt_dir, "D_{}.pth".format(global_step)
                ),
            )
        else:
            utils.save_checkpoint(
                ...
                epoch,
                os.path.join(
                    hps.s2_ckpt_dir, "G_{}.pth".format(233333333333)
                ),
            )
            utils.save_checkpoint(
                net_d,
                ...
                epoch,
                os.path.join(
                    hps.s2_ckpt_dir, "D_{}.pth".format(233333333333)
                ),
            )
