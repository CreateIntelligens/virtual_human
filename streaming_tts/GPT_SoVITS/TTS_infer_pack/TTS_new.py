...
class TTS_Config:
    ...
    def __init__(self, configs: Union[dict, str]=None, no_save_file: bool = False):
        # 設置默認配置文件路徑
        configs_base_path:str = "GPT_SoVITS/configs/"
        os.makedirs(configs_base_path, exist_ok=True)
        self.no_save_file = no_save_file
        ...
        # self.t2s_weights_path = os.path.join("../gpt-sovits", self.configs.get("t2s_weights_path", None).lstrip('./'))
        # self.vits_weights_path = os.path.join("../gpt-sovits", self.configs.get("vits_weights_path", None).lstrip('./'))
        self.bert_base_path = self.configs.get("bert_base_path", None)
        self.cnhuhbert_base_path = self.configs.get("cnhuhbert_base_path", None)

        # 檢查並設置默認值
        if (self.t2s_weights_path in [None, ""]) or (not os.path.exists(self.t2s_weights_path)):
        # if (self.t2s_weights_path in [None, ""]) or (not os.path.exists(os.path.join("../gpt-sovits", self.t2s_weights_path.lstrip('./')))):
            self.t2s_weights_path = self.default_configs['t2s_weights_path']
            print(f"fall back to default t2s_weights_path: {self.t2s_weights_path}")
        if (self.vits_weights_path in [None, ""]) or (not os.path.exists(self.vits_weights_path)):
        # if (self.vits_weights_path in [None, ""]) or (not os.path.exists(os.path.join("../gpt-sovits", self.vits_weights_path.lstrip('./')))):
    ...
    def save_configs(self, configs_path:str=None)->None:
        if self.no_save_file:  # 如果設置了不保存，直接返回
            return
        configs={
    ...
    @classmethod
    def from_dict(cls, config: dict, no_save_file: bool = False):
        """直接從字典創建配置
        
        Args:
            config (dict): 配置字典，可以是簡單字典或帶custom/default結構的字典
            
        Returns:
            TTS_Config: 配置實例
        """
        instance = cls(None, no_save_file=no_save_file)  # 創建空實例並傳入 no_save_file 參數
        
        # 如果是帶有custom的結構，使用custom部分
        if "custom" in config:
        ...
        # 設置各個屬性
        instance.device = config.get("device", torch.device("cpu"))
        instance.is_half = config.get("is_half", False)
        instance.t2s_weights_path = config.get("t2s_weights_path")
        instance.vits_weights_path = config.get("vits_weights_path")
        # instance.t2s_weights_path = os.path.join("../gpt-sovits", config.get("t2s_weights_path").lstrip('./'))
        # instance.vits_weights_path = os.path.join("../gpt-sovits", config.get("vits_weights_path").lstrip('./'))
        instance.bert_base_path = config.get("bert_base_path")
        instance.cnhuhbert_base_path = config.get("cnhuhbert_base_path")
        
        # 檢查並設置默認值
        if (instance.t2s_weights_path in [None, ""]) or (not os.path.exists(instance.t2s_weights_path)):
        # if (instance.t2s_weights_path in [None, ""]) or (not os.path.exists(os.path.join("../gpt-sovits", instance.t2s_weights_path.lstrip('./')))):
            instance.t2s_weights_path = instance.default_configs['t2s_weights_path']
            print(f"fall back to default t2s_weights_path: {instance.t2s_weights_path}")
        if (instance.vits_weights_path in [None, ""]) or (not os.path.exists(instance.vits_weights_path)):
        # if (instance.vits_weights_path in [None, ""]) or (not os.path.exists(os.path.join("../gpt-sovits", instance.vits_weights_path.lstrip('./')))):
            instance.vits_weights_path = instance.default_configs['vits_weights_path']
            print(f"fall back to default vits_weights_path: {instance.vits_weights_path}")
        if (instance.bert_base_path in [None, ""]) or (not os.path.exists(instance.bert_base_path)):
            instance.bert_base_path = instance.default_configs['bert_base_path']
            print(f"fall back to default bert_base_path: {instance.bert_base_path}")
        if (instance.cnhuhbert_base_path in [None, ""]) or (not os.path.exists(instance.cnhuhbert_base_path)):
            instance.cnhuhbert_base_path = instance.default_configs['cnhuhbert_base_path']
            print(f"fall back to default cnhuhbert_base_path: {instance.cnhuhbert_base_path}")
...
class TTS:
    ...
    def init_vits_weights(self, weights_path: str):
        ...
        # weights_path = os.path.join("../gpt-sovits", weights_path.lstrip('./'))
        print(f"Loading VITS weights from {weights_path}")
    ...
    def init_t2s_weights(self, weights_path: str):
        ...
        try:
            ...
            print(f"Error loading t2s model: {e}")
            raise e
    ...
    def _set_prompt_semantic(self, ref_wav_path:str):
        ...
        with torch.no_grad():
            ...
            if (wav16k.shape[0] > 160000 or wav16k.shape[0] < 48000):
                raise OSError(i18n("參考音頻在3~10秒範圍外，請更換！"))
    ...
    def cleanup_resources(self):
        """清理所有模型資源"""
        try:
            if hasattr(self, 't2s_model'):
                del self.t2s_model
                self.t2s_model = None
            if hasattr(self, 'vits_model'):
                del self.vits_model
                self.vits_model = None
    ...
    def run(self, inputs:dict):
            ...
            inputs (dict): 
                {
                    ...
                    "text_split_method": "cut0",  # str. text split method, see text_segmentation_method.py
                    "batch_size": 1,              # int. batch size for inference
                    "batch_threshold": 0.75,      # float. threshold for batch splitting.
                    "split_bucket": True,         # bool. whether to split batch into buckets
                    "return_fragment": False,     # bool. step by step return the audio fragment
                    "speed_factor": 1.0,          # float. control the speed of the synthesized audio
                    "fragment_interval": 0.3,     # float. to control the interval of audio fragment
        ...
        # if "./output" in inputs.get("ref_audio_path", ""):
        #     ref_audio_path = os.path.join("../gpt-sovits", inputs.get("ref_audio_path", "").lstrip('./'))
        ...
        if not no_prompt_text:
            ...
            print(i18n("實際輸入的參考文本:"), prompt_text)
        ...
        try:
            ...
            for item in data:
                ...
                print(i18n("前端處理後的文本(每句):"), norm_text)
                if no_prompt_text :
                    prompt = None
                else:
    ...
    def audio_postprocess(self, 
        ...
        for i, batch in enumerate(audio):
            for j, audio_fragment in enumerate(batch):
                max_audio=torch.abs(audio_fragment).max()#簡單防止16bit爆音
        ...
        sf.write(f'./audio/audio.wav',audio,sr)
        
        return sr, audio


def speed_change(input_audio:np.ndarray, speed:float, sr:int):
    # 將 NumPy 數組轉換為原始 PCM 流
