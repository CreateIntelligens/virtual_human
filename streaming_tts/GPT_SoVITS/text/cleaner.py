...
# --- 新增：從 custom_map.json 讀取自定義映射 ---
custom_map_path = os.path.join(os.path.dirname(__file__), "custom_map.json")
with open(custom_map_path, encoding="utf-8") as f:
    custom_map = json.load(f)
# --------------------------------------------

def clean_text(text, language):
    # --- 應用自定義映射 ---
    for src, tgt in custom_map.items():
