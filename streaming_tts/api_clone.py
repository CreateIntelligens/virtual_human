...
parser.add_argument("-c", "--tts_config", type=str, default="GPT_SoVITS/configs/tts_infer_clone.yaml", help="tts_infer路径")
parser.add_argument("-a", "--bind_addr", type=str, default="0.0.0.0", help="default: 127.0.0.1")
parser.add_argument("-p", "--port", type=int, default="1981", help="default: 9880")
...
if config_path in [None, ""]:
    config_path = "GPT-SoVITS/configs/tts_infer_clone.yaml"
...
async def tts_get_endpoint(
                        ...
                        prompt_text: str = "禅宗假设要讲修行来讲四个字就讲完就迫向前行", # ""
                        top_k:int = 5,
                        top_p:float = 1,
                        temperature:float = 1,
...
if __name__ == "__main__":
    try:
        uvicorn.run(app="api_clone:APP", host=host, port=port, workers=1, log_level="info")
    except Exception as e:
        traceback.print_exc()
        os.kill(os.getpid(), signal.SIGTERM)
        exit(0)
