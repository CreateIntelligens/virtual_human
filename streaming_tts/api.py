...
from inference_webui import inference_api
...
def handle_new(text, text_lang, ref_audio_path, prompt_text, prompt_lang,text_split_method="按标点符号切"):
    ...
    with torch.no_grad():
        gen = inference_api(text, text_lang, 
              ref_audio_path, prompt_text, 
              prompt_lang, top_k=4, 
              top_p=0.8, temperature=0.8, 
              text_split_method=text_split_method, batch_size=20, 
              speed_factor=1.0, ref_text_free=False,
              split_bucket=True,fragment_interval=0.3,seed=-1, keep_random=True, parallel_infer=True,
...
async def change_model(sweight="SoVITS_weights/Keira_e8_s96.pth",gweight="GPT_weights/Keira-e15.ckpt"):

    tts_config = TTS_Config("GPT_SoVITS/configs/tts_infer.yaml")

    tts_config.t2s_weights_path = gweight
    tts_config.vits_weights_path = sweight

    tts_pipline = TTS(tts_config)
    
    return JSONResponse([sweight,gweight],status_code=200)
