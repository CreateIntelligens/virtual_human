...
if __name__ == '__main__':
    ...
    # requests.get("https://u393077-9dad-9627fa89.westb.seetacloud.com:8443/set_gpt_weights?weights_path=GPT_weights/嫂子.ckpt")
    # requests.get("https://u393077-9dad-9627fa89.westb.seetacloud.com:8443/set_sovits_weights?weights_path=SoVITS_weights/嫂子.pth")

    # headers = {'Content-Type': 'application/json'}

    # gpt = {"text":text,"text_lang":"zh","ref_audio_path":"./audio/嫂子#zh#咱们不管是送人，还是自用，都特别划算，而且给大家用的，都是这种专用的泡沫箱.wav","prompt_lang":"zh","prompt_text":"咱们不管是送人，还是自用，都特别划算，而且给大家用的，都是这种专用的泡沫箱","text_split_method":"cut5","batch_size":5,"speed_factor":1.0}

    # response = requests.post("https://u393077-9dad-9627fa89.westb.seetacloud.com:8443/",data=json.dumps(gpt),headers=headers)

    # response = requests.get(f"https://u393077-9dad-9627fa89.westb.seetacloud.com:8443/?text={text}&text_lang=zh&ref_audio_path=./audio/嫂子#zh#咱们不管是送人，还是自用，都特别划算，而且给大家用的，都是这种专用的泡沫箱.wav&prompt_lang=zh&prompt_text=咱们不管是送人，还是自用，都特别划算，而且给大家用的，都是这种专用的泡沫箱&text_split_method=cut5&batch_size=1&media_type=wav&speed_factor=1.0")


    # audio_data = response.content

    # with open(f"{text}_new.wav","wb") as f:
    #     f.write(audio_data)
