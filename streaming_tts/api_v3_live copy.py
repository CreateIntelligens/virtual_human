...
async def transcribe(audio: UploadFile = File(...)):
    ...
    try:
        ...
        try:
            ...
            print(f"音頻轉換成功，保存於: {converted_file_name}")
            ...
            print(f"轉錄完成，文本長度: {len(input_text)}")
            ...
            print(f"FFmpeg轉換錯誤: {e}")
            print(f"FFmpeg錯誤輸出: {e.stderr.decode() if e.stderr else '無'}")
            raise Exception(f"音頻轉換失敗: {str(e)}")
            
    except Exception as e:
        # 錯誤處理與記錄
        import traceback
        print(f"轉錄處理錯誤: {str(e)}")
        traceback.print_exc()
        return JSONResponse(
            status_code=500,
            content={"error": str(e)}
        ...
        for temp_file in [original_file_name, converted_file_name]:
            if os.path.exists(temp_file):
                try:
                    os.unlink(temp_file)
                    print(f"已刪除臨時檔案: {temp_file}")
                except Exception as e:
                    print(f"刪除臨時檔案失敗 {temp_file}: {str(e)}")
