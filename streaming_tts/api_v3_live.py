...
async def transcribe(audio: UploadFile = File(...)):
    ...
    try:
        ...
        try:
            ...
            url = "https://ddgsrvchat.aicreate360.com/custom_service_with_language?language=chinese"
            headers = {
...
async def transcribe(audio: UploadFile = File(...), conversation_id: str = Form("U03cd17c02ef4a297c2c2a910e5b6219f")):
    try:
        # 初始化Google Speech client
        client = speech.SpeechClient()
        print(f"[{datetime.now()}] 開始轉錄...")
        ...
        # 調用聊天機器人API
        url = "https://devocr.aitago.tw/custom_service"
        headers = {
            "Content-Type": "application/json",
            "Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyX2lkIjoxLCJlbWFpbCI6ImFkbWluQGFpY3JlYXRlMzYwLmNvbSIsInJvbGUiOiJhZG1pbiIsImlhdCI6MTYwNjQ4NjU5Mn0.0j5z1E1l1z8Vr6ZtX9Dz4J6G1m6wD3jV5b8W7Xw5Y2c"
        }
        ...
        print(f"[{datetime.now()}] 聊天機器人回應: {response.text}")
        response_json = response.json()
        message = response_json['message'][0]['content']
        conversation_id = response_json['message'][0]['status']['conversation_id']
        print(f"[{datetime.now()}] 會話ID: {conversation_id}")
...
async def transcribe(audio: UploadFile = File(...)):
    try:
        ...
        # 調用聊天機器人API
        url = "https://ddgsrvchat.aicreate360.com/custom_service_with_language?language=english"
        headers = {
            "Content-Type": "application/json",
            "Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyX2lkIjoxLCJlbWFpbCI6ImFkbWluQGFpY3JlYXRlMzYwLmNvbSIsInJvbGUiOiJhZG1pbiIsImlhdCI6MTYwNjQ4NjU5Mn0.0j5z1E1l1z8Vr6ZtX9Dz4J6G1m6wD3jV5b8W7Xw5Y2c"
        }
...
async def transcribe(audio: UploadFile = File(...), conversation_id: str = Form("U03cd17c02ef4a297c2c2a910e5b6219f")):
    try:
        ...
        input_text = re.sub(r'。$', '', input_text)
        print(f"[{datetime.now()}] 轉錄完成，文本長度: {len(input_text)}")
        print(f"[{datetime.now()}] 原始文本: {input_text}")
        ...
        # 調用聊天機器人API
        url = " https://cs01-line.ai360.workers.dev/api/chat"
        headers = {
            "Content-Type": "application/json"
        }
        ...
        print(f"[{datetime.now()}] 原先的會話ID: {conversation_id}")
        print(f"[{datetime.now()}] 呼叫聊天機器人")
        response = requests.post(url, headers=headers, json=payload, verify=False)
        print(f"[{datetime.now()}] 聊天機器人回應: {response.text}")
        response_json = response.json()
        
        # 從回應中提取 AI 訊息 (最後一條 type 為 'ai' 的訊息)
        ai_messages = [msg for msg in response_json['messages'] if msg['type'] == 'ai']
        if ai_messages:
            message = ai_messages[-1]['content']
        else:
            message = "抱歉，我無法理解您的問題。"
        
        # conversation_id 保持不變,使用原有的 conversation_id
        print(f"[{datetime.now()}] 後來的會話ID: {conversation_id}")
...
async def ai_chat(request_data: Dict[str, str]):
    try:
        # 檢查 text 欄位
        input_text = request_data.get("text")
        conversation_id = request_data.get("conversation_id", "U03cd17c02ef4a297c2c2a910e5b6219f")
        if not input_text:
            raise HTTPException(status_code=400, detail="必須提供 text 參數")
...
async def ai_chat(request_data: Dict[str, str]):
    try:
        ...
        print(f"[{datetime.now()}] 原始文本: {input_text}")
        converted_input = converter.convert(input_text)
        print(f"[{datetime.now()}] 轉換後的文本: {converted_input}")
