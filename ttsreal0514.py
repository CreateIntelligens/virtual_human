...
class VoitsTTS(BaseTTS):
    ...
    def gpt_sovits(self, text, reffile, reftext,language, server_url) -> Iterator[bytes]:
        ...
        # #── 不分大小寫，支援「ODM」「O D M」→「歐低M」、「OEM」「O E M」→「歐依M」──
        # text = re.sub(r'\bO\s*D\s*M\b', '歐低M', text, flags=re.IGNORECASE)
        # text = re.sub(r'\bO\s*E\s*M\b', '歐依M', text, flags=re.IGNORECASE)

        #── 不分大小寫，支援「ODM」「O D M」→「歐低M」、「OEM」「O E M」→「歐依M」──
        # 把 \b 換成只針對 ASCII 字母的 lookaround，避免中文也被算作 \w
        text = re.sub(
            r'(?<![A-Za-z])O\W*D\W*M(?![A-Za-z])',
            '歐低M', text, flags=re.IGNORECASE
        )
        text = re.sub(
            r'(?<![A-Za-z])O\W*E\W*M(?![A-Za-z])',
            '歐依M', text, flags=re.IGNORECASE
        )
